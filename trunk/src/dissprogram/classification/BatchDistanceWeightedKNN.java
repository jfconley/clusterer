 ...
 * @author jconley
 */
public class BatchDistanceWeightedKNN implements BatchKNN{
    ...
    public double[] classifyProb(double[] dataPoint){
        ...
        System.arraycopy(currentDists, 0, copy, 0, input.length);  //using arraycopy to make sure the distArray doesn't get messed up.
        Arrays.sort(copy);        
        for (int j = 0; j < classes.length; j++){
            stringCounts[j] = stringCount(currentNeighbors, copy, classes[j]);
        }
    ...
    public String[] neighborClasses(double[] dataPoint){
        ...
        currentDists = new double[input.length];
        for (int i = 0; i < input.length; i++){
            currentDists[i] = DissUtils.euclideanDist(dataPoint, input[i].getArray());
        }
        double[] copy = new double[input.length];
        System.arraycopy(currentDists, 0, copy, 0, input.length);  //using arraycopy to make sure the distArray doesn't get messed up.
        Arrays.sort(copy);
        ...
        for (int j = 0; j < input.length; j++){
            if (currentDists[j] <= thresh){
                //it's less than the threshold, so get the exact position
                int blob = 0;
                boolean found = false;
                while (!(found) && (blob < maxK)){
                    if ((copy[blob] == currentDists[j]) && (rankArray[blob].equalsIgnoreCase("empty"))){  //do the "empty" thing in case of ties.
